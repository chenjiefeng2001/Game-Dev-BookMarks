...
本节描述了一个低开销实时分析模块的架构和实现，该模块支持性能计数器组织，使多个消费者能够协同工作。从长远来看，实现一个干净的 profiling 系统所花费的时间是一项明智的投资。而且，与其他事情一样，我们越早规划，就会越容易。
### Profiling 基础
基本的性能分析机制很简单：***在目标代码的开始处和结束处分别获取一个时间戳。将第一个时间戳减去第二个时间戳***，就这样，代码运行的时间就出来了。我们需要一个高分辨率计数器——Windows 多媒体计时器及其毫秒级分辨率是远远不够的。如果平台是 PC 上的 Windows，我们可以使用两个高分辨率的 API 调用：`QueryPerformanceCounter`、`manceCounter` 和 `QueryPerformanceFrequency`。然而，由于开销这些函数的效率相当高，我们将自己编写，只需几行内联汇编：
```C++
...
```
要将这个数字转换为秒，我们需要知道计数器的频率。在这种情况下，它等于每秒的 CPU 周期数。我们可以在启用计数器时测量一次——取一个时间样本，休眠至少 500 毫秒，然后取另一个样本。
> 请注意，如果目标平台是游戏机，则可以使用类似的计数器。

### 商用工具
性能调优确实是选择合适工具可以带来巨大差异的案例。有很多经过时间考验的商用工具可以在 PC 上运行时采样应用程序，然后在离线状态下按模块、按函数等方式查看分析数据。

### 为何要自己动手？
- 理由一：***基于帧的分析。游戏通常具有较高的帧间一致性，但在几秒钟内可能会发生巨大变化***。游戏引擎会经历许多可能不同的瓶颈，这些瓶颈只能通过逐帧分析来真正识别。查看在整个样本积累期间的分析分解情况，间隔时间给出的视图并不准确。基于帧的分析允许一次专注于一个问题。
- 理由二：它可以在任何时间和任何地方进行。在 PC 游戏开发周期的结束时，有人可能会面临性能问题，这些问题只会在某人的兄弟的机器上，在奇特的星期二显现出来。通常有大量这类问题。它们可能会耗费大量时间，并且很容易导致发布日期延误。尽管这类问题在 PC 游戏中很独特，但主机游戏仍然需要处理“在第三关的角落发射导弹使游戏缓慢运行”这类问题。一旦问题被理解，找到解决方案通常是比较容易的部分。如果我们能走到那台测试机器旁，弹出一组计数器，我们就能快速锁定罪魁祸首。
- 理由三：**可定制性**。现代游戏引擎非常复杂。能够忽略引擎中的所有其他模块，只专注于正在处理的模块是一项强大的功能。此外，只有实际执行工作的人员才能按照他们想要的方式组织数据。
### 性能分析模块需求
- 需求一：***允许用户快速准确地分析应用程序***。
- 需求二：***非侵入式***。当采样和显示结果的成本成为死帧时间的显著部分时，它实际上可能会改变应用程序在系统中的行为。通常，减慢 CPU 速度往往会隐藏显卡引起的停顿。虽然即使是非常小的百分比在某些罕见情况下也可能大幅改变游戏性能，**但作为一般规则，当性能分析器启用时，它应占不到总 CPU 周期的 5%。当禁用时，它应远低于 1%**。
- 需求三：***允许多个用户独立地在其各自的系统上工作，而无需担心其他引擎模块***。
### 架构与实现
一个性能计数器管理器跟踪所有活跃和非活跃的计数器。这些计数器被组织成具有相似类型的组（例如，模型渲染、世界渲染、人工智能、物理），并且这些组会一起启用和禁用。这支持了多个组独立工作的概念，并采用易于理解的分组方式。组之所以有用，有两个原因：*一是快速确定是否需要采样计数器，二是启用和禁用显示计数器组*。我们将使用**四字符代码（FourCC）作为组 ID**，并使用完整的文本字符串作为计数器名称。
整个系统被组织成一个模块，并与系统的其他部分接口。基本组件是一个由组 ID 标识的计数器（它的FourCC)及其字符串名称。每个计数器在创建时被赋予一个唯一的整数 ID。在典型使用中，游戏代码在初始化时创建计数器，并将开始/停止计数器调用放在要分析的代码周围。
以下为基本功能单元接口如下所示：
```C++
class IPerfCounterMan{
	public:
	//添加新的计数器(返回ID，0为失效)
	int32 AddCounter(uint32 CounterGroup, const char* szCounterName);
	//忘记你的计数器 ID？
...
```
`StopCounter` 计算开始计数器（StartCounter）和停止计数器（StopCounter）调用之间的差值，并保持累计总和。在 `DrawCounters` 中，所有累计计数器都会被清除。同时还会维护一个最大值，并在` DrawCounters`的帧结束时设置该值。假设我们的引擎有一个接受文本命令的调试控制台。这是一种非常方便的方式来启用和禁用计数器组，并允许自定义显示。

允许在计数器显示中尽可能多地配置非常有帮助。我们很可能不希望每帧都刷新计数器显示（每 30 帧更新一次就足够了），但根据调试的内容，自定义刷新时间的能力会非常方便。此外，显示当前百分比和上次显示以来的最大百分比是有用的。

条形图是显示结果的好方法。它让用户能快速了解数字，而且编写的代码也不难。能够在百分比和实际时间（以毫秒为单位）之间切换，将时间或百分比显示为文本值，以及自动缩放坐标轴也非常有用。要注意频繁切换坐标轴缩放，尤其是在没有某种警告的情况下，因为这很可能会让人们感到困惑。
### 实现细节
性能计数器管理器的接口应该是灵活且易于使用的。**使用配置管理器的用户通常会发现在简单地调用加计数器配合完整的字符串，一次性获取ID并启动，而不是在某一个一次性初始化点保存计数器ID**。提供这种机制可以在进行快速分析时提供帮助。然而，它的效率并不高，在帧中多次调用会迅速累积。此外，提供一个可以在调用 StartCounter 的构造函数和 StopCounter 的析构函数（当它失去焦点时）中放置在函数开头的类，可以是一种方便的计数器监控方法。
在编写分析管理器时，最好提供某种#define 宏来完全移除分析器。在追求游戏极致性能时，分析代码往往是首先被移除的东西。我们需要提供 `AddCounter`、`StartCounter `和`topCounter` 的宏，在`#ifdefdnan%t `时完全编译出去。
此外，最好使用颜色作为视觉提示。当显示计数器时，如果每行使用不同的颜色，更容易阅读。
### 数据分析
> 务必分析发布版本，因为它的瓶颈可能与调试版本差异很大。如果目标平台是 PC，选择两到三种典型的系统配置（从低端到高端）并分别进行分析也是一个好主意。不同系统配置之间的瓶颈可能会有很大差异。


游戏应该在出现性能问题的区域以及典型游戏过程中进行分析。我们必须将问题分解，一次专注于一件事情，并关注那些能带来最大效益的区域。仅仅因为一个函数调用次数最多或占用最多 CPU 时间，并不意味着我们应该只关注那里。通常，我们唯一可以与我们的周期时间进行比较的就是我们的期望，而现实的期望通常只有通过经验才能获得。

分析器本身也应该进行分析。如果分析行为具有侵入性，它会改变游戏的行为。分析器的绘制例程周围应该有一个计数器。
### 实现注释
所描述的模块已在多个平台上实现。然而，其***中部分需要平台相关的函数。实际的时间戳查询和绘制函数很可能需要在平台相关的代码中实现，因此最好围绕这些函数设计一层抽象***。所描述的实现使用了一套调试几何和文本（这具有平台相关...）在绘制代码中实现依赖性（dependent implementation），以便使其可以跨平台工作。您可能需要编写一个宏来创建您的四个字符代码值，因为许多编译器都不支持它们。
这个系统也可以用来对游戏服务器进行长时间的性能分析，以检测问题。所有的计数器都通过一个源进行，因此数据可以轻松地被过滤并保存到磁盘。
...
>如果你之前线性地编写过程序，你将很容易理解本文介绍的方法的重要性。如果你的游戏编程经验始于 Windows，那么你可能会觉得消息泵是游戏编程的自然环境，但一旦你尝试线性编程，你将永远不会回到消息泵。与庞大的有限状态机相比，它要清晰得多，也更容易遵循和调试。如果你开始以更线性方式工作，你可以节省大量的设计、编程、调试时间和思考。

### 更新世界
一个现代的游戏通常都有一个名为`UpdateWorld`的函数，它位于应用程序的消息队列中，并在没有接收到任何消息时被调用。在初次尝试时，编写一个 UpdateWorld 函数可以非常简单：***所有应用程序变量、表面和接口都已经被初始化，现在我们只需要更新和渲染它们。这应该是一个简单的任务，但前提是我们要计划游戏将只有一个屏幕、没有过场动画、没有菜单，也没有选项***

问题是 UpdateWorld 最终必须完成并返回到消息队列，以便我们可以处理来自系统的消息。这阻止了我们停留在连续的 for 循环中。