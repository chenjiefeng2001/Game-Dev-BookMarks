...
> ***尽量减少堆分配，并在紧密循环中永不从堆中分配。***

没有游戏引擎可以完全避免动态内存分配，因此大多数游戏引擎**实现了一个或多个自定义分配器**。自定义分配器比操作系统堆分配器具有更好的性能特征，原因有以下几个方面：
- 自定义内存分配器可以从预分配的内存块（它本身使用 `malloc()` 或 `new` 分配，或声明为*全局变量*）满足请求。**这允许它在用户模式下运行，完全避免进入操作系统的上下文切换成本**
- 通过对其使用模式做出各种假设，自定义分配器可以比通用堆分配器高效得多。
...
##### 基于栈的分配器
目前主流的分配方式：
关卡加载-> 分配内存-> 开始游玩加载的关卡(在这期间几乎不会再分配新的内存，仅仅使用预先分配好的内存)-> 完成关卡->进入下一关-> 卸载上一关卡的内存，并为下一关进行新的分配 
栈分配器非常容易实现。**我们只需使用 malloc()或全局 new 分配一个大的连续内存块，或者通过声明一个字节数组全局数组**（在这种情况下，内存实际上是从可执行文件的 BSS 段分配的）。维护一个指向栈顶的指针。此指针以下的所有内存地址都被认为是已使用的，而它以上的所有地址都被认为是空闲的。顶指针被初始化为栈中最低的内存地址。每个分配请求只需将指针向上移动请求的字节数。最近分配的块可以通过将顶指针向下移动块的大小来释放。
